              ...
              <div className="p-6">
                {modalMode === 'view' ? (
                  isLoadingDetail ? (
                    <div className="flex items-center justify-center py-12">
                      <div className="text-center">
                        ...
                        <p className="text-slate-600 font-medium">Loading dataset details...</p>
                      </div>
                    </div>
                  ) : (
                  ...
                  <div className="bg-gradient-to-r from-slate-50 to-amber-50/50 p-4 rounded-2xl border border-slate-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.name}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Slug</label>
                        <p className="p-3 bg-white rounded-xl text-gray-900 font-mono shadow-sm">{selectedDataset?.slug}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Status</label>
                        <p className={`inline-block px-4 py-2 rounded-xl font-semibold ${getStatusBadge(selectedDataset?.status)}`}>
                          {selectedDataset?.status}
                        </p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Access Level</label>
                        <p className={`inline-block px-4 py-2 rounded-xl font-semibold ${getAccessBadge(selectedDataset?.access_level)}`}>
                  ...
                  <div>
                    ...
                    <p className="p-4 bg-gray-50 rounded-xl text-gray-900 leading-relaxed">{selectedDataset?.description}</p>
                  </div>
                  
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Tagline</label>
                    <p className="p-3 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl text-gray-900 border border-amber-200">{selectedDataset?.tagline || 'N/A'}</p>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-blue-50 to-indigo-50/50 p-4 rounded-2xl border border-blue-200">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="bg-white p-4 rounded-xl shadow-sm">
                        <p className="text-xs text-slate-600 mb-1 font-semibold">Samples</p>
                        <p className="text-2xl font-bold text-slate-900">{selectedDataset?.samples?.toLocaleString()}</p>
                      </div>
                      <div className="bg-white p-4 rounded-xl shadow-sm">
                        <p className="text-xs text-slate-600 mb-1 font-semibold">Size</p>
                        <p className="text-2xl font-bold text-slate-900">{formatFileSize(selectedDataset?.size || 0)}</p>
                      </div>
                      <div className="bg-white p-4 rounded-xl shadow-sm">
                        <p className="text-xs text-slate-600 mb-1 font-semibold">Downloads</p>
                        <p className="text-2xl font-bold text-slate-900">{selectedDataset?.download_count || 0}</p>
                      </div>
                      <div className="bg-white p-4 rounded-xl shadow-sm">
                        <p className="text-xs text-slate-600 mb-1 font-semibold">Version</p>
                        <p className="text-2xl font-bold text-slate-900">{selectedDataset?.version}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-r from-purple-50 to-pink-50/50 p-4 rounded-2xl border border-purple-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        ...
                        <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.format}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">License</label>
                        <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.license}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Type</label>
                        <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.technical_specs?.type || 'N/A'}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Access</label>
                        <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.technical_specs?.access || 'N/A'}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Last Update</label>
                        <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.technical_specs?.lastUpdate || 'N/A'}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Gradient</label>
                        <p className="p-3 bg-white rounded-xl text-gray-900 font-mono text-xs shadow-sm">{selectedDataset?.gradient || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {selectedDataset?.statistics && (
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50/50 p-4 rounded-2xl border border-green-200">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <div>
                          <label className="block text-sm font-semibold text-gray-700 mb-2">Avg Image Size</label>
                          <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.statistics?.avgImageSize || 'N/A'}</p>
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-gray-700 mb-2">Quality Score</label>
                          <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.statistics?.qualityScore || 0}</p>
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-gray-700 mb-2">Total Annotations</label>
                          <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.statistics?.totalAnnotations?.toLocaleString() || 0}</p>
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-gray-700 mb-2">Avg Images/Category</label>
                          <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.statistics?.avgImagesPerCategory?.toLocaleString() || 0}</p>
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-gray-700 mb-2">Max Images/Category</label>
                          <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.statistics?.maxImagesPerCategory?.toLocaleString() || 0}</p>
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-gray-700 mb-2">Min Images/Category</label>
                          <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.statistics?.minImagesPerCategory?.toLocaleString() || 0}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedDataset?.key_features && selectedDataset.key_features.length > 0 ? (
                        ...
                        <p className="text-gray-500 italic">No key features listed</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedDataset?.use_cases && selectedDataset.use_cases.length > 0 ? (
                        ...
                        <p className="text-gray-500 italic">No use cases listed</p>
                      )}
                    </div>
                  </div>

                  {/* Categories */}
                  {selectedDataset?.categories && selectedDataset.categories.length > 0 && (
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
                        <svg className="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                  ...
                  {/* Sample Images */}
                  {selectedDataset?.sample_images && selectedDataset.sample_images.length > 0 && (
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
                        <svg className="w-5 h-5 text-pink-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        </svg>
                        Sample Images ({selectedDataset.sample_images.length})
                      </label>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        {selectedDataset.sample_images.map((image, index) => (
                          <div key={index} className="bg-gray-50 p-2 rounded-xl border border-gray-200">
                            <img src={image} alt={`Sample ${index + 1}`} className="w-full h-32 object-cover rounded-lg mb-2" onError={(e) => e.target.style.display = 'none'} />
                            <p className="text-xs text-gray-600 truncate font-mono">{image}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="p-3 bg-gray-50 rounded-xl text-gray-900">{selectedDataset?.source || 'N/A'}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">File URL</label>
                      {selectedDataset?.file_url ? (
                        <a href={selectedDataset.file_url} target="_blank" rel="noopener noreferrer" className="p-3 bg-blue-50 rounded-xl text-blue-600 hover:text-blue-700 block truncate underline">
                          {selectedDataset.file_url}
                        </a>
                      ) : (
                        <p className="p-3 bg-gray-50 rounded-xl text-gray-500 italic">No file URL</p>
                      )}
                    </div>
                  </div>
                  ...
                  {selectedDataset?.citation && (
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">Citation</label>
                      <pre className="p-4 bg-gray-900 text-green-400 rounded-xl text-xs font-mono overflow-x-auto">{selectedDataset.citation}</pre>
                    </div>
                  )}
                  ...
                  <div className="bg-gradient-to-r from-slate-50 to-gray-50 p-4 rounded-2xl border border-slate-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="p-3 bg-white rounded-xl text-gray-900 font-mono text-xs shadow-sm">{selectedDataset?.id}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Creator Name</label>
                        <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.creator_name || 'N/A'}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Created At</label>
                        <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.created_at ? new Date(selectedDataset.created_at).toLocaleString() : 'N/A'}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Updated At</label>
                        <p className="p-3 bg-white rounded-xl text-gray-900 shadow-sm">{selectedDataset?.updated_at ? new Date(selectedDataset.updated_at).toLocaleString() : 'N/A'}</p>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Created By (ID)</label>
                        <p className="p-3 bg-white rounded-xl text-gray-900 font-mono text-xs shadow-sm">{selectedDataset?.created_by || 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                </div>
