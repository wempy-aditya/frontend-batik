    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-amber-50/30 to-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          <div className="flex flex-col gap-4">
            <div>
              ...
              <p className="text-slate-600 mt-1 text-sm sm:text-base">Manage user roles, status, and permissions</p>
            </div>
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Username</th>
                ...
                {filteredUsers.length > 0 ? (
                  filteredUsers.map((user) => (
                    <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-slate-900 font-mono text-sm">{user.username || '-'}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border ${roleColors[user.role] || roleColors.registered}`}>
            ...
            <div className="bg-slate-50 px-6 py-4 border-t border-slate-200 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2 bg-white border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  Previous
                </button>
                <button
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden transform transition-all animate-slideUp">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-slate-600 text-sm mt-1">
                      {modalMode === 'view' ? 'View user information' : 'Update user role and status'}
                    </p>
                  </div>
                  <button
                    onClick={closeModal}
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="p-3 bg-white rounded-xl text-slate-900 shadow-sm">{formData.name}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-2">Username</label>
                      <p className="p-3 bg-white rounded-xl text-slate-900 font-mono shadow-sm">{formData.username || '-'}</p>
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-semibold text-slate-700 mb-2">Email</label>
                      <p className="p-3 bg-white rounded-xl text-slate-900 shadow-sm">{formData.email}</p>
                    </div>
                  </div>
                </div>
                ...
                {modalMode === 'edit' && (
                  <div className="bg-gradient-to-r from-amber-50 to-orange-50/50 p-4 rounded-2xl border border-amber-200">
                    <h3 className="text-lg font-bold text-slate-900 mb-4">Role & Status Management</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">Tier ID</label>
                        <input
                          type="number"
                          name="tier_id"
                      ...
                      <div className="md:col-span-2 space-y-3">
                        <label className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-200 hover:border-amber-500 cursor-pointer transition-colors">
                          ...
                          />
                          <div>
                            <span className="font-semibold text-slate-900">Active Status</span>
                            <p className="text-xs text-slate-600">User can login and access the system</p>
                          </div>
                        </label>
                        ...
                        <label className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-200 hover:border-purple-500 cursor-pointer transition-colors">
                          ...
                          />
                          <div>
                            <span className="font-semibold text-slate-900">Superuser Privileges</span>
                            <p className="text-xs text-slate-600">Grant full administrative access</p>
                          </div>
                        </label>
                      </div>
                    </div>
                ...
                {modalMode === 'view' && (
                  <div className="bg-gradient-to-r from-blue-50 to-indigo-50/50 p-4 rounded-2xl border border-blue-200">
                    <h3 className="text-lg font-bold text-slate-900 mb-4">Permissions & Status</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">Role</label>
                        <span className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold border ${roleColors[formData.role] || roleColors.registered}`}>
                      ...
                      <div>
                        ...
                        <span className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold ${
                          ...
                          <span className={`w-2 h-2 rounded-full ${formData.is_active ? 'bg-green-500' : 'bg-red-500'}`}></span>
                          {formData.is_active ? 'Active' : 'Inactive'}
                        </span>
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">Superuser</label>
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">Tier ID</label>
                        <p className="p-3 bg-white rounded-xl text-slate-900 shadow-sm">{formData.tier_id || '-'}</p>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-mono text-slate-900 break-all">{selectedUser.id}</p>
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1">Created At</label>
                        <p className="text-slate-900">{selectedUser.created_at ? new Date(selectedUser.created_at).toLocaleString() : '-'}</p>
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1">Updated At</label>
                        <p className="text-slate-900">{selectedUser.updated_at ? new Date(selectedUser.updated_at).toLocaleString() : '-'}</p>
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 mb-1">Deleted</label>
                        <p className="text-slate-900">{selectedUser.is_deleted ? 'Yes' : 'No'}</p>
                      </div>
                    </div>
                  </div>
                )}
