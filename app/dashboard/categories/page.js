  ...
  const handleDelete = async (categoryId) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/categories/${categoryId}?type=${selectedType}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-amber-50/30 to-slate-50">
          ...
          <div className="flex flex-wrap gap-3">
            {categoryTypes.map((type) => (
              ...
              >
                ...
                {selectedType === type.value && (
                  <span className="bg-white/25 backdrop-blur-sm text-xs font-bold px-2.5 py-1 rounded-lg">
                    {categories.length}
                  </span>
                )}
              </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden transform transition-all animate-slideUp">
