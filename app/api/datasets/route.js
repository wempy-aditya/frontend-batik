...
const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || 'https://spmb1.wempyaw.com';
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: 'Failed to fetch datasets', error: errorData },
    ...
    return NextResponse.json(
      { message: 'Failed to fetch datasets', error: error.message },
      { status: 500 }
    );
  }
}
...
export async function POST(request) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { message: 'Failed to create dataset', error: errorData },
        { status: response.status }
      );
    }
