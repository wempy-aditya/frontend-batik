export async function GET(request) {
  try {
    ...
    const response = await fetch(`https://spmb1.wempyaw.com/api/v1/news/?offset=${offset}&limit=${limit}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return Response.json(
        { message: 'Failed to get news from external API' },
...
export async function POST(request) {
  try {
    ...
    // Forward request to external API
    const response = await fetch('https://spmb1.wempyaw.com/api/v1/news/', {
      method: 'POST',
      headers: {
        'Authorization': authHeader,
        'Content-Type': 'application/json',
