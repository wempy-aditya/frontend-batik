...
export default function ResultPage() {
  ...
  const fetchPatchImages = async (dataset, patchIndexes) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/patches/${dataset}`);
      if (response.ok) {
        const data = await response.json();
        if (data.success && data.patches) {
          const patchA = data.patches.find((p) => p.index === patchIndexes[0]);
          const patchB = data.patches.find((p) => p.index === patchIndexes[1]);
          setPatchImages({
            patchA: patchA ? `${API_BASE_URL}/${patchA.path}` : null,
            patchB: patchB ? `${API_BASE_URL}/${patchB.path}` : null,
          });
        }
      }
    } catch (err) {
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    {generatedImages[modelName] && generatedImages[modelName] !== "error" && (
                      <a
                        href={generatedImages[modelName]}
                        download={`batik-${modelName}-${Date.now()}.jpg`}
